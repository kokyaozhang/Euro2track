        ...
        <div class="max-w-max mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                <button wire:click="create()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3">Record New Calibrations </button>
                    @if($isOpen)
                    @include('livewire.creategcalibration')
                @endif

                    @foreach($dent as $wah)
                        @if($wah->State == 'In Calibration')
                <button wire:click="createimp()" class="bg-fuchsia-600 hover:bg-fuchsia-800 text-white font-bold py-2 px-4 rounded my-3">Service complete</button>
                    @if($isimpOpen)
                            @include('livewire.reportcbbo')
                    @endif
                            @elseif($wah->State == 'Intact')
                            <button wire:click="createimp2()" class="bg-fuchsia-600 hover:bg-fuchsia-800 text-white font-bold py-2 px-4 rounded my-3">Report Calibrations</button>
                            @if($isimp2Open)
                            @include('livewire.creterptcb')
                            @endif
                        @endif
                    @endforeach
                            ...
                            <td class="border py-2 p-4">
                                <div class="p-2">
                                    ...
                                    <div>
                                        ...
                                        <form action="{{ route('file.keep2',['id'=>$fieldequip->id]) }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <input
                                                type="file"
                                                name="file"
